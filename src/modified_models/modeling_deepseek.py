...
class DeepseekV2Attention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        activations = {"layer_idx": self.layer_idx}
        ...
        activations["attention_outputs"] = attn_output
        ...
        B, H, Q, Dv  = attn_output.shape                # batch, heads, seq, head-dim
        d_model      = self.hidden_size                 # same as self.o_proj.out_features

        W = self.o_proj.weight                          # [d_model, H*Dv]
        W_heads = (
            W.view(d_model, H, Dv)                      # [d_model, H, Dv]
            .permute(1, 2, 0)                          # [H, Dv, d_model]
        )

        # per_head_proj : [B, H, Q, d_model]
        per_head_proj = torch.einsum("bhqd,hdm->bhqm", attn_output, W_heads)
        ...
        return attn_output, activations, past_key_value
